...
Debug complex issues using specialized analysis. Provide the error message, stack trace, or symptom description.
...
### 3. For FFmpeg / subprocess errors
- Check the `[KTV-RC]` and `[KTV-FF]` lines in the debug log
- Verify all input file paths exist before the command runs
- Check FFmpeg exit code and last stderr line

### 4. For yt-dlp / download errors
- Check if yt-dlp version is current (`py -m pip show yt-dlp`)
- Verify `--remote-components ejs:github` flag is present
- Examine `[DL-CMD]` lines in debug log

### 5. For UI / tkinter errors
...
**Implementation**: specific code changes with file:line references
